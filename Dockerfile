...
WORKDIR /app
RUN apt update -q && apt-get -y dist-upgrade && apt -y install --no-install-recommends --no-install-suggests nginx libnginx-mod-stream swaks ca-certificates nano wget curl && rm -rf /var/lib/{apt,dpkg,cache,log} && apt clean
